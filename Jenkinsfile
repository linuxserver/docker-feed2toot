pipeline {
  ...
  environment {
    ...
    EXT_PIP = 'feed2toot'
    BUILD_VERSION_ARG = 'FEED2TOOT_VERSION'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-feed2toot'
    CONTAINER_NAME = 'feed2toot'
    DOCKERHUB_IMAGE = 'linuxserver/feed2toot'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/feed2toot'
    PR_DOCKERHUB_IMAGE = 'lspipepr/feed2toot'
    DIST_IMAGE = 'alpine'
    MULTIARCH='true'
    CI='false'
    CI_WEB='false'
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              set -e
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/feed2toot") | .uuid')
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/feed2toot",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-feed2toot",\
                       "backendUrl":"https://ghcr.io/linuxserver/feed2toot",\
                       "publicUrl":"https://lscr.io/linuxserver/feed2toot"}' || :
              else
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker build \
          ...
          --label \"org.opencontainers.image.documentation=https://docs.linuxserver.io/images/docker-feed2toot\" \
          --label \"org.opencontainers.image.source=https://github.com/linuxserver/docker-feed2toot\" \
